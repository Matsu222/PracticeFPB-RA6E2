...
/* Actual period: 0.005 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0xf424,
  .duty_cycle_counts = 0x7a12, .source_div = (timer_source_div_t) 3, .channel = 0, .p_callback = g_timer1_callback,
...
/* Actual period: 0.67108864 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x10000,
  .duty_cycle_counts = 0x8000, .source_div = (timer_source_div_t) 10, .channel = 0, .p_callback = g_timer0_callback,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
